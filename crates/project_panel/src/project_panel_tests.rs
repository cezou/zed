...
/// Mirrors what real multi-buffer views (`ProjectDiagnosticsEditor`,
/// `ProjectDiff`, etc.) do: the workspace `Item` is a thin wrapper that holds
/// an inner `Editor` and re-emits its events. The wrapper itself has no static
/// project path, so the project panel's auto-reveal is entirely driven by the
/// inner editor publishing the cursor's buffer.
...
async fn test_autoreveal_follows_multibuffer_selection(cx: &mut gpui::TestAppContext) {
    ...
    // Wrap the multibuffer editor in an `Item` whose own `active_project_path`
    // is `None`, mirroring real multibuffer views (`ProjectDiagnosticsEditor`,
    // `ProjectDiff`, etc.). Auto-reveal must rely on the inner editor
    // publishing the cursor's buffer, and unrelated wrapper-level events
    // (title/dirty changes) must not clobber that selection.
    workspace.update_in(cx, |workspace, window, cx| {
    ...
    // reaches `Workspace::active_item_path_changed`. Because the wrapper's own
    // `active_project_path` is `None`, the workspace must leave the
    // already-published selection alone instead of clobbering it.
